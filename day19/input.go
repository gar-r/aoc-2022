...
var example = parseInput(exampleRaw)
var input = parseInput(inputRaw)

var parseRe = regexp.MustCompile(`.*(\d+) ore.* (\d+).* (\d+) ore and (\d+) clay.* (\d+) ore and (\d+) obsidian.`)

func parseInput(s string) []Blueprint {
	res := make([]Blueprint, 0)
	matches := parseRe.FindAllStringSubmatch(s, -1)
	for _, match := range matches {
		res = append(res, Blueprint{
			OreRobot:  {Ore: mustParseInt(match[1])},
			ClayRobot: {Ore: mustParseInt(match[2])},
			ObsidianRobot: {
				Ore:  mustParseInt(match[3]),
				Clay: mustParseInt(match[4]),
			},
			GeodeRobot: {
				Ore:      mustParseInt(match[5]),
				Obsidian: mustParseInt(match[6]),
			},
		})
	}
	return res
