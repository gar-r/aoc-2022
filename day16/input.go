...
var parseRe = regexp.MustCompile(`Valve ([A-Z]+) has flow rate=(\d+); tunnels? leads? to valves? (.+)`)

func parse(s string) []*Valve {
	match := parseRe.FindAllStringSubmatch(s, -1)
	res := make([]*Valve, len(match))
	for i, m := range match {
		res[i] = parseMatch(m)
	}
	return res
...
func parseMatch(m []string) *Valve {
